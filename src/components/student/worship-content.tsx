...
export function WorshipContent() {
  ...
  const fetchRecord = async (date: Date) => {
    try {
      setLoading(true);
      const dateStr = format(date, "yyyy-MM-dd");
      const res = await fetch(`/api/worship-tracker?date=${dateStr}`);
      if (res.ok) {
  ...
  const saveRecord = async (updatedRecord: WorshipRecord) => {
    try {
      ...
      // Find the student profile to retrieve its ID
      const res = await fetch("/api/worship-tracker", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          studentId: undefined, // Will default to self on the server for Student
            ...
            <div className="flex justify-between items-center pb-4 border-b border-gray-100 mb-6">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  Record your 5 daily Salaah for {format(selectedDate, "MMMM dd")}
                </p>
              </div>
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-center pb-4 border-b border-indigo-900/50">
              <div>
                ...
                <p className="text-xs text-indigo-200 mt-0.5">Send blessings on Prophet Muhammad ﷺ</p>
              </div>
            </div>
